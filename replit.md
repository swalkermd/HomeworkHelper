...
## Recent Changes (Nov 14, 2025) - Previous Session
- **FIXED: Responsive Theme Tokens** - Refactored typography and spacing tokens to dynamically update on device rotation using `useResponsiveTheme()` hook. Fixed critical bug where orientation changes were ignored (tokens were frozen at app load). SolutionScreen now recomputes layouts when device rotates between landscape/portrait. Maintained backward compatibility for static imports.
- **FIXED: Step Reveal State Reset** - Fixed bug where viewing a second solution would show all steps instantly instead of progressive reveal. Added state reset (`setRevealedSteps(0)`, `setAllRevealed(false)`) when new solution loads, restoring pacing UX for every solution.
- **NEW: Solution Validation Callouts** - Added visual warnings when solution data is incomplete (missing problem statement, steps, or final answer). Yellow-highlighted callouts appear at top of Problem Statement section listing specific issues detected.
- **NEW: Refined Solution Screen Layout** - Added `SectionHeader` component with title/subtitle for clear organization. Polished metadata display (subject, difficulty) and enhanced final answer styling with descriptive labels. Updated typography scale for better legibility.
- **FIXED: Handwritten Final Answers** - Enhanced hybrid OCR pipeline to detect handwritten problems and format final answers in Caveat handwriting font with color highlights. STEM path now passes original image to GPT-4o analysis (alongside Mistral OCR text) for visual handwriting detection. Both STEM and non-STEM paths apply `[handwritten:[red:answer]]` formatting for handwritten problems, making solutions feel personal and relatable. Zero latency impact (reuses existing GPT-4o call).
- **NEW: Landscape Mode Layout Optimization** - Fixed image review screen to eliminate scrolling in landscape orientation. Image preview dynamically scales to 220px max height in landscape (vs 400px portrait), with reduced spacing and compact padding throughout. All buttons remain visible without scrolling.
