...
- **NEW: WolframAlpha Math Verification** - Replaced Gemini with WolframAlpha API for math problem verification using computational ground truth. Math-first routing: problems classified by subject/keywords use WolframAlpha Short Answers API (attempt 1) before AI verification. Multi-part answer extraction handles labeled parts (a), (b), (c) with individual WolframAlpha queries per part. Verification pipeline: Math problems → WolframAlpha (2000 calls/month) → GPT-4o (attempt 2) → GPT-4o retry (attempt 3). Non-math problems skip WolframAlpha and use GPT-4o directly. Rate limiting uses mutex-locked file counter in `data/wolfram-usage.json` with automatic monthly reset. WolframAlpha provides definitive numerical/symbolic verification for calculus, algebra, and physics, eliminating false positives from AI-only verification. Falls back to GPT-4o if WolframAlpha cannot solve or returns inconclusive results.
